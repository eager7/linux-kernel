...
int i2s_disable_tx(i2s_config_type* ptri2s_config);
...
static int i2s_release(struct inode *inode, struct file *filp)
{
   ...
   // printk("==================close=================\n");
    ...
    if (0 == i2s_disable_flag)
    {
        i2s_disable_tx(ptri2s_config);
    }
    i2s_disable_flag = 0;
...
void i2s_dma_tx_handler(u32 dma_ch)
{
	...
	if(pi2s_config->bTxDMAEnable==0) 
	{
		if(pi2s_config->end_cnt != 0)
		{
			...
			pi2s_config->end_cnt --;
        	printk("end_cnt = %d, r_idx = %d, w_idx = %d\n", \
                pi2s_config->end_cnt, pi2s_config->tx_r_idx, pi2s_config->tx_w_idx);
		}
		else
		{
			...
			printk("====tx_stop=%d========\n", pi2s_config->tx_stop_cnt);
			//if (pi2s_config->tx_stop_cnt%3 == 0)
	...
	{
		/* Enable PAUSE */
        //if (pi2s_config->tx_isr_cnt % 100 == 0)
        //    printk("=========i2s pause=============\n");
        
        //pi2s_config->tx_r_idx = pi2s_config->tx_w_idx = 0;
...
#endif
{
    ...
    //leihj add for CMD FLUSH and GET_CACHE
    int idx_ret = 0;
    int cache_size = 0;
	...
	case I2S_TX_ENABLE:
		...
		//if(ptri2s_config->bRxDMAEnable==0)  //changed by wing 2015/04/09
		MSG("i2s_clock_enable wing 2015/04/09\r\n");
	...
	case I2S_TX_DISABLE:
		...
		{
            printk("============tx_stop_cnt:%d\n", ptri2s_config->tx_stop_cnt);
			interruptible_sleep_on(&(ptri2s_config->i2s_tx_qh));
		}
	...
	case I2S_RX_DISABLE:
		...
		break;
	case I2S_PUT_AUDIO:
        if (ptri2s_config->tx_pause_en)
        {
            printk("=======i2s alread pause, but app write date to i2s.=======\n");
            printk("=======i2s alread pause, but app write date to i2s.=======\n");
            printk("=======i2s alread pause, but app write date to i2s.=======\n");
            printk("=======i2s alread pause, but app write date to i2s.=======\n");
            break;
	...
	case I2S_TX_PAUSE:
		...
		printk("* tx_pause_en = 1 *\n");
		spin_unlock_irqrestore(&ptri2s_config->lock, flags);
                break;
	case I2S_TX_RESUME:
		spin_lock_irqsave(&ptri2s_config->lock, flags);
                ptri2s_config->tx_pause_en = 0;
		printk("# tx_pause_en = 0 #\n");
		spin_unlock_irqrestore(&ptri2s_config->lock, flags);
                break;
	case I2S_RX_STOP:
		spin_lock_irqsave(&ptri2s_config->lock, flags);
	...
	case I2S_DEBUG_RESET:
		...
		break;			
    //leihj add for xi'an test
    case I2S_GET_CACHE:
        idx_ret = ptri2s_config->tx_w_idx >= ptri2s_config ->tx_r_idx \
                    ?   (ptri2s_config->tx_w_idx - ptri2s_config ->tx_r_idx)\
                    :   (ptri2s_config->tx_r_idx - ptri2s_config ->tx_w_idx);
        cache_size = idx_ret * I2S_PAGE_SIZE;
        put_user(cache_size, (int *)arg);
        
        break;
        
    case I2S_BUFFER_FLUSH:
#if 0
        spin_lock_irqsave(&ptri2s_config->lock, flags);
        ptri2s_config->tx_r_idx = ptri2s_config->tx_w_idx; 
        spin_unlock_irqrestore(&ptri2s_config->lock, flags);
...
int i2s_disable_tx(i2s_config_type* ptri2s_config)
{
    ...
    spin_lock_irqsave(&ptri2s_config->lock, flags);
    printk("i2s_disable_tx\n");
    /* Handle tx end data */
    ptri2s_config->bTxDMAEnable = 0;
    i2s_dma_tx_end_handle(ptri2s_config);
    spin_unlock_irqrestore(&ptri2s_config->lock, flags);

    while(ptri2s_config->tx_stop_cnt<10)
    {
        printk("while counter tx_stop_cnt: %d\n", ptri2s_config->tx_stop_cnt);
        interruptible_sleep_on(&(ptri2s_config->i2s_tx_qh));
    }
    spin_lock_irqsave(&ptri2s_config->lock, flags);
    i2s_reset_tx_param(ptri2s_config);
